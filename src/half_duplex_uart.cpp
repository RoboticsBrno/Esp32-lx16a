...
{
    UART_CHECK((uart_num < UART_NUM_MAX), "uart_num error", ESP_FAIL);
    *(data_bit) = (uart_word_length_t)UART[uart_num]->conf0.bit_num;
    return ESP_OK;
}
...
esp_err_t uart_get_stop_bits(uart_port_t uart_num, uart_stop_bits_t* stop_bit)
{
    ...
    if (UART[uart_num]->rs485_conf.dl1_en == 1 && UART[uart_num]->conf0.stop_bit_num == UART_STOP_BITS_1) {
        ...
        (*stop_bit) = (uart_stop_bits_t)UART[uart_num]->conf0.stop_bit_num;
    }
    return ESP_OK;
}
...
esp_err_t uart_get_hw_flow_ctrl(uart_port_t uart_num, uart_hw_flowcontrol_t* flow_ctrl)
{
    ...
    if(UART[uart_num]->conf1.rx_flow_en) {
        val = (uart_hw_flowcontrol_t)(val | UART_HW_FLOWCTRL_RTS);
    }
    if(UART[uart_num]->conf0.tx_flow_en) {
        val = (uart_hw_flowcontrol_t)(val | UART_HW_FLOWCTRL_CTS);
    }
    (*flow_ctrl) = val;
    return ESP_OK;
}
...
esp_err_t uart_set_pin(uart_port_t uart_num, int tx_io_num, int rx_io_num, int rts_io_num, int cts_io_num)
{
    ...
    if(rx_io_num >= 0) {
        ...
        gpio_set_pull_mode((gpio_num_t)rx_io_num, (gpio_pull_mode_t)GPIO_PULLUP_ONLY);
        gpio_set_direction((gpio_num_t)rx_io_num, (gpio_mode_t)GPIO_MODE_INPUT);
...
static void IRAM_ATTR uart_rx_intr_handler_default(void *param)
{
    ...
    while(uart_intr_status != 0x0) {
        ...
        if(uart_intr_status & UART_TX_DONE_INT_ST_M) {
            ...
            if (p_uart->rx_buffer_full_flg == false) {
                ...
                if (uart_intr_status & UART_AT_CMD_CHAR_DET_INT_ST_M) {
                    ...
                    if (p_uart->uart_select_notif_callback) {
                        p_uart->uart_select_notif_callback((uart_port_t)uart_num, UART_SELECT_READ_NOTIF, &HPTaskAwoken);
                    }
            ...
            UART_ENTER_CRITICAL_ISR(&uart_selectlock);
            if (p_uart->uart_select_notif_callback) {
                p_uart->uart_select_notif_callback((uart_port_t)uart_num, UART_SELECT_ERROR_NOTIF, &HPTaskAwoken);
            }
            UART_EXIT_CRITICAL_ISR(&uart_selectlock);
