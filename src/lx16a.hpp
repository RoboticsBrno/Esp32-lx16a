...
struct Packet {
    ...
    static Packet move(Id id, uint16_t position, uint16_t time) {
        return Packet(id, Command::SERVO_MOVE_TIME_WRITE, position & 0xFF, position >> 8, time & 0xFF, time >> 8);
    }

    static Packet limitAngle(Id id, uint16_t low, uint16_t high) {
        return Packet(id, Command::SERVO_ANGLE_LIMIT_WRITE, low & 0xFF, low >> 8, high & 0xFF, high >> 8);
    }

    static Packet setId(Id id, Id newId) { return Packet(id, Command::SERVO_ID_WRITE, newId); }

    static Packet getId(Id id) { return Packet(id, Command::SERVO_ID_READ); }
...
public:
    ...
    static Packet move(Id id, lx16a::Angle pos) {
        float position = pos.deg();
    ...
    static Packet limit(Id id, lx16a::Angle b, lx16a::Angle t) {
        int bottom = b.deg();
        int top = t.deg();
