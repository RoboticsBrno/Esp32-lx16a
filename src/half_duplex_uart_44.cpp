...
typedef struct {
    ...
    uart_mode_t uart_mode;              /*!< UART controller actual mode set by half_duplex::uart_set_mode() */
...
esp_err_t uart_set_baudrate(uart_port_t uart_num, uint32_t baud_rate)
{
    ...
    ESP_RETURN_ON_ERROR(uart_get_sclk_freq(UART_SCLK_DEFAULT, &sclk_freq), UART_TAG, "Invalid src_clk");
...
int uart_pattern_pop_pos(uart_port_t uart_num)
{
    ...
    if (pat_pos != NULL && pat_pos->rd != pat_pos->wr) {
        ...
        half_duplex::uart_pattern_dequeue(uart_num);
    }
    UART_EXIT_CRITICAL(&(uart_context[uart_num].spinlock));
    return pos;
}
...
{
    return half_duplex::uart_disable_intr_mask(uart_num, UART_INTR_CMD_CHAR_DET);
}

esp_err_t uart_enable_rx_intr(uart_port_t uart_num)
{
    return half_duplex::uart_enable_intr_mask(uart_num, UART_INTR_RXFIFO_FULL | UART_INTR_RXFIFO_TOUT);
}

esp_err_t uart_disable_rx_intr(uart_port_t uart_num)
{
    return half_duplex::uart_disable_intr_mask(uart_num, UART_INTR_RXFIFO_FULL | UART_INTR_RXFIFO_TOUT);
}

esp_err_t uart_disable_tx_intr(uart_port_t uart_num)
{
    return half_duplex::uart_disable_intr_mask(uart_num, UART_INTR_TXFIFO_EMPTY);
}
...
esp_err_t uart_set_pin(uart_port_t uart_num, int tx_io_num, int rx_io_num, int rts_io_num, int cts_io_num)
{
    ...
    if (tx_io_num >= 0 && !uart_try_set_iomux_pin(uart_num, tx_io_num, SOC_UART_TX_PIN_IDX)) {
        ...
        gpio_set_level((gpio_num_t)tx_io_num, 1);
    ...
    if (rx_io_num >= 0 && !uart_try_set_iomux_pin(uart_num, rx_io_num, SOC_UART_RX_PIN_IDX)) {
        ...
        gpio_set_pull_mode((gpio_num_t)rx_io_num, GPIO_PULLUP_ONLY);
        gpio_set_direction((gpio_num_t)rx_io_num, GPIO_MODE_INPUT);
    ...
    if (rts_io_num >= 0 && !uart_try_set_iomux_pin(uart_num, rts_io_num, SOC_UART_RTS_PIN_IDX)) {
        ...
        gpio_set_direction((gpio_num_t)rts_io_num, GPIO_MODE_OUTPUT);
    ...
    if (cts_io_num >= 0  && !uart_try_set_iomux_pin(uart_num, cts_io_num, SOC_UART_CTS_PIN_IDX)) {
        ...
        gpio_set_pull_mode((gpio_num_t)cts_io_num, GPIO_PULLUP_ONLY);
        gpio_set_direction((gpio_num_t)cts_io_num, GPIO_MODE_INPUT);
...
esp_err_t uart_param_config(uart_port_t uart_num, const uart_config_t *uart_config)
{
    ...
    half_duplex::uart_module_enable(uart_num);
    ...
    uart_hal_set_sclk(&(uart_context[uart_num].hal), UART_SCLK_DEFAULT);
#if ESP_IDF_VERSION >= 0x050000
        ...
        if (uart_intr_status & UART_INTR_TXFIFO_EMPTY) {
            ...
            if (p_uart->tx_waiting_fifo == true && p_uart->tx_buf_size == 0) {
                ...
                while (tx_fifo_rem) {
                    ...
                    if (p_uart->tx_len_tot > 0 && p_uart->tx_ptr && p_uart->tx_len_cur > 0) {
                        // To fill the TX FIFO.
                        uint32_t send_len = half_duplex::uart_enable_tx_write_fifo(uart_num, (const uint8_t *) p_uart->tx_ptr,
            ...
            if (p_uart->rx_buffer_full_flg == false) {
                ...
                if (uart_intr_status & UART_INTR_CMD_CHAR_DET) {
                    ...
                    pat_idx = half_duplex::uart_find_pattern_from_last(p_uart->rx_data_buf, rx_fifo_len - 1, pat_chr, pat_num);
                } else {
                ...
                if (pdFALSE == xRingbufferSendFromISR(p_uart->rx_ring_buf, p_uart->rx_data_buf, p_uart->rx_stash_len, &HPTaskAwoken)) {
                    ...
                    if (uart_event.type == UART_PATTERN_DET) {
                        ...
                        if (rx_fifo_len < pat_num) {
                            ...
                            half_duplex::uart_pattern_enqueue(uart_num, p_uart->rx_buffered_len - (pat_num - rx_fifo_len));
                        } else {
                            half_duplex::uart_pattern_enqueue(uart_num,
                                                 pat_idx <= -1 ?
...
// #endif
                    ...
                    if (uart_intr_status & UART_INTR_CMD_CHAR_DET) {
                        if (rx_fifo_len < pat_num) {
                            ...
                            half_duplex::uart_pattern_enqueue(uart_num, p_uart->rx_buffered_len + pat_idx);
                        }
                    }
                    p_uart->rx_buffered_len += p_uart->rx_stash_len;
                    UART_EXIT_CRITICAL_ISR(&(uart_context[uart_num].spinlock));
...
int uart_tx_chars(uart_port_t uart_num, const char *buffer, uint32_t len)
{
    ...
    tx_len = (int)half_duplex::uart_enable_tx_write_fifo(uart_num, (const uint8_t *) buffer, len);
    xSemaphoreGive(p_uart_obj[uart_num]->tx_mux);
    return tx_len;
}
...
static int uart_tx_all(uart_port_t uart_num, const char *src, size_t size, bool brk_en, int brk_len)
{
    ...
    if (p_uart_obj[uart_num]->tx_buf_size > 0) {
        ...
        while (size > 0) {
            ...
            size -= send_size;
            offset += send_size;
            half_duplex::uart_enable_tx_intr(uart_num, 1, UART_EMPTY_THRESH_DEFAULT);
        }
    } else {
        while (size) {
            //semaphore for tx_fifo available
            if (pdTRUE == xSemaphoreTake(p_uart_obj[uart_num]->tx_fifo_sem, (portTickType)portMAX_DELAY)) {
                uint32_t sent = half_duplex::uart_enable_tx_write_fifo(uart_num, (const uint8_t *) src, size);
                if (sent < size) {
...
int uart_write_bytes(uart_port_t uart_num, const void *src, size_t size)
{
    ...
    return half_duplex::uart_tx_all(uart_num, (const char*)src, size, 0, 0);
}
...
int uart_write_bytes_with_break(uart_port_t uart_num, const void *src, size_t size, int brk_len)
{
    ...
    return half_duplex::uart_tx_all(uart_num, (const char*)src, size, 1, brk_len);
}
...
static bool uart_check_buf_full(uart_port_t uart_num)
{
    if (p_uart_obj[uart_num]->rx_buffer_full_flg) {
        ...
        if (res == pdTRUE) {
            ...
            half_duplex::uart_reenable_intr_mask(p_uart_obj[uart_num]->uart_num, UART_INTR_RXFIFO_TOUT | UART_INTR_RXFIFO_FULL);
            return true;
        }
    }
    return false;
...
int uart_read_bytes(uart_port_t uart_num, void *buf, uint32_t length, TickType_t ticks_to_wait)
{
    ...
    while (length) {
        ...
        half_duplex::uart_pattern_queue_update(uart_num, len_tmp);
        ...
        if (p_uart_obj[uart_num]->rx_cur_remain == 0) {
            ...
            half_duplex::uart_check_buf_full(uart_num);
        }
    }
...
esp_err_t uart_flush_input(uart_port_t uart_num)
{
    ...
    while (true) {
        if (p_uart->rx_head_ptr) {
            ...
            half_duplex::uart_pattern_queue_update(uart_num, p_uart->rx_cur_remain);
            UART_EXIT_CRITICAL(&(uart_context[uart_num].spinlock));
            p_uart->rx_ptr = NULL;
        ...
        half_duplex::uart_pattern_queue_update(uart_num, size);
        UART_EXIT_CRITICAL(&(uart_context[uart_num].spinlock));
    ...
    half_duplex::uart_reenable_intr_mask(uart_num, UART_INTR_RXFIFO_TOUT | UART_INTR_RXFIFO_FULL);
...
{
    uart_obj_t *uart_obj = (uart_obj_t*)heap_caps_calloc(1, sizeof(uart_obj_t), UART_MALLOC_CAPS);
    if (!uart_obj) {
        return NULL;
    }
#if 1 // CONFIG_UART_ISR_IN_IRAM
    ...
    if (event_queue_size > 0) {
        uart_obj->event_queue = xQueueCreateStatic(event_queue_size, sizeof(uart_event_t),
                                (uint8_t*)uart_obj->event_queue_storage, (StaticQueue_t*)uart_obj->event_queue_struct);
        if (!uart_obj->event_queue) {
            goto err;
        }
    }
    if (tx_buffer_size > 0) {
        uart_obj->tx_ring_buf = xRingbufferCreateStatic(tx_buffer_size, RINGBUF_TYPE_NOSPLIT,
                                (uint8_t*)uart_obj->tx_ring_buf_storage, (StaticRingbuffer_t*)uart_obj->tx_ring_buf_struct);
        if (!uart_obj->tx_ring_buf) {
            goto err;
        }
    }
    uart_obj->rx_ring_buf = xRingbufferCreateStatic((size_t)rx_buffer_size, RINGBUF_TYPE_BYTEBUF,
                            (uint8_t*)uart_obj->rx_ring_buf_storage, (StaticRingbuffer_t*)uart_obj->rx_ring_buf_struct);
    uart_obj->rx_mux = xSemaphoreCreateMutexStatic((StaticQueue_t*)uart_obj->rx_mux_struct);
    uart_obj->tx_mux = xSemaphoreCreateMutexStatic((StaticQueue_t*)uart_obj->tx_mux_struct);
    uart_obj->tx_brk_sem = xSemaphoreCreateBinaryStatic((StaticQueue_t*)uart_obj->tx_brk_sem_struct);
    uart_obj->tx_done_sem = xSemaphoreCreateBinaryStatic((StaticQueue_t*)uart_obj->tx_done_sem_struct);
    uart_obj->tx_fifo_sem = xSemaphoreCreateBinaryStatic((StaticQueue_t*)uart_obj->tx_fifo_sem_struct);
...
err:
    half_duplex::uart_free_driver_obj(uart_obj);
    return NULL;
}
...
esp_err_t uart_driver_install(uart_port_t uart_num, int rx_buffer_size, int tx_buffer_size, int event_queue_size, QueueHandle_t *uart_queue, int intr_alloc_flags)
{
    ...
    if (p_uart_obj[uart_num] == NULL) {
        p_uart_obj[uart_num] = half_duplex::uart_alloc_driver_obj(event_queue_size, tx_buffer_size, rx_buffer_size);
        if (p_uart_obj[uart_num] == NULL) {
    ...
    uart_hal_clr_intsts_mask(&(uart_context[uart_num].hal), UART_LL_INTR_MASK);
    r = half_duplex::uart_isr_register(uart_num, uart_rx_intr_handler_default, p_uart_obj[uart_num], intr_alloc_flags, &p_uart_obj[uart_num]->intr_handle);
    if (r != ESP_OK) {
        goto err;
    }
...
esp_err_t uart_driver_delete(uart_port_t uart_num)
{
    ...
    half_duplex::uart_free_driver_obj(p_uart_obj[uart_num]);
    p_uart_obj[uart_num] = NULL;

#if SOC_UART_SUPPORT_RTC_CLK
    uart_sclk_t sclk = (uart_sclk_t)0;
    uart_hal_get_sclk(&(uart_context[uart_num].hal), &sclk);
    if (sclk == UART_SCLK_RTC) {
        rtc_clk_disable(uart_num);
    }
#endif
    half_duplex::uart_module_disable(uart_num);
    return ESP_OK;
}
...
#endif //#if (defined(ESP_IDF_VERSION) && ESP_IDF_VERSION >= 0x040400)
